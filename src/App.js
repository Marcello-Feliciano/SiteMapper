...
export default function App() {
  ...
  // placed markers (x,y normalized). cone-capable markers also have rotation (deg)
  ...
  // single correct startRotate (no duplicate)
  const startRotate = (markerId, e) => {
    if (!overlayRef.current) return;
    e.stopPropagation();
    e.preventDefault(); // prevent native drag behavior while rotating
  ...
  const onPointerMove = (e) => {
    ...
    setPlaced((list) => list.map((m) => (m.id === dragState.current.id ? { ...m, x: clampedX, y: clampedY } : m)));
  };
    ...
    <div style={{ fontFamily: "Inter, system-ui, Arial, sans-serif", background: "#f5f7fb", minHeight: "100vh" }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <input ref={importInputRef} type="file" accept="image/png,image/jpeg,application/json" onChange={handleImportChange} style={{ display: "none" }} />
        </div>
      </header>

      {/* Marker palette */}
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", padding: "12px 16px", background: "#eef3fb", borderBottom: "1px solid #dde5f1" }}>
        ...
        {markerTypes.map((m) => {
          ...
          return (
            ...
            >
              {m.iconSrc && <img src={m.iconSrc} alt={m.label} style={{ width: 24, height: 24 }} />}
            </button>
          );
        })}
        {selectedTypeId && (
          <button onClick={() => setSelectedTypeId(null)} style={{ marginLeft: 6, padding: "6px 10px", borderRadius: 8, border: "1px solid #c9d6ea", background: "#fff", cursor: "pointer" }}>
            Deselect
          </button>
        )}
      </div>
      ...
      <main style={{ padding: 16, display: "flex", justifyContent: "center" }}>
        {!imageSrc ? (
          <div style={{ textAlign: "center", marginTop: 48 }}>
            <div onClick={handleImportClick} style={{ width: 360, background: "#fff", borderRadius: 14, boxShadow: "0 8px 24px rgba(16,24,40,.08)", padding: 24, cursor: "pointer" }}>
              ...
              <div style={{ color: "#667085", fontSize: 14, marginBottom: 14 }}>Upload a building layout or floorplan to start adding markers</div>
              <div style={{ border: "2px dashed #cfd8e3", borderRadius: 10, padding: 16, color: "#6b7280", fontSize: 13 }}>Tap to select file (PNG, JPG, or JSON)</div>
            </div>
          ...
          <div ref={stageRef} style={{ position: "relative", background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 8px 24px rgba(16,24,40,.06)", padding: 8, maxWidth: "min(95vw, 1200px)", overflow: "auto", touchAction: "none" }}>
            <div ref={imageWrapRef} style={{ position: "relative", display: "inline-block", width: "100%" }}>
              <img id="floorplan-image" ref={imgRef} alt="Floorplan" src={imageSrc} style={{ display: "block", width: "100%", height: "auto", borderRadius: 8, userSelect: "none", pointerEvents: "none" }} draggable={false} />
              ...
              <div
                ...
                style={{ position: "absolute", inset: 0, borderRadius: 8, pointerEvents: "auto" }}
              >
                {placed.map((m) => {
                  ...
                  const content = type?.iconSrc ? (
                    <img src={type.iconSrc} alt={type?.label || "icon"} style={{ width: 32, height: 32, minWidth: 32, minHeight: 32, objectFit: "contain", pointerEvents: "none" }} />
                  ) : null;
                  ...
                  return (
                    <div
                      ...
                      style={{ position: "absolute", left: `${m.x * 100}%`, top: `${m.y * 100}%`, transform: "translate(-50%, -50%)", cursor: "grab", userSelect: "none", touchAction: "none", background: "transparent", padding: 0, boxShadow: "none" }}
                      title={showCone ? "Click to rotate • Drag to move • Double-click to delete" : "Drag to move • Double-click to delete"}
                    >
                      {/* Cone */}
                      {showCone && (
                        <div style={{ position: "absolute", left: "50%", top: "50%", transform: `translate(-50%,-50%) rotate(${rotation}deg)`, transformOrigin: "50% 50%", pointerEvents: "none" }}>
      ...
      {/* Export modal */}
      {showExportModal && (
        <div onClick={() => setShowExportModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(2,6,23,.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: 360, background: "#fff", borderRadius: 14, boxShadow: "0 16px 48px rgba(16,24,40,.2)", padding: 18, display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ fontWeight: 700, fontSize: 16 }}>Export filename</div>
            <input value={exportFilename} onChange={(e) => setExportFilename(e.target.value)} placeholder="layout" autoFocus style={{ padding: "10px 12px", borderRadius: 10, border: "1px solid #e2e8f0", outline: "none", fontSize: 14 }} />
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <button onClick={() => setShowExportModal(false)} style={{ padding: "8px 12px", borderRadius: 10, background: "#f3f4f6", border: "1px solid #e5e7eb", cursor: "pointer" }}>
                Cancel
              </button>
              <button onClick={() => { exportJSONandPNG(exportFilename); setShowExportModal(false); }} style={{ padding: "8px 12px", borderRadius: 10, background: "#1976d2", color: "#fff", border: "none", cursor: "pointer", fontWeight: 600 }}>
                Export
              </button>
            </div>
